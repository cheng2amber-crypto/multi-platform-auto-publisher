# publishers/eastmoney.py
from playwright.sync_api import sync_playwright
import os
import time

def publish_eastmoney(content: str):
    ...
    with sync_playwright() as p:
        ...
        context_path = "cookies/eastmoney.json"
        context = browser.new_context(storage_state=context_path) if os.path.exists(context_path) else browser.new_context()
        page = context.new_page()

        page.goto("https://mp.eastmoney.com/collect/pc_article/index.html#/")
        page.wait_for_load_state("networkidle")

        if "login" in page.url:
            raise Exception("未登录东方财富，请先手动登录")
